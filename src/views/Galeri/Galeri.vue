      ...
      <b-container>
        ...
        <b-row>
          <b-col md="4" class="mt-4" v-for="item in pelatihans" :key="item.id">
            <div class="box">
              <img :src="setSrc(item.bannerPelatihan)" alt="" style="border-top-left-radius: 10px;border-top-right-radius:10px;width:100%">
              ...
              <div class="content">
                <h4 class="title">{{item.judulPelatihan}}</h4>
                      ...
                      <b-table-simple small borderless class="mb-0">
                        <b-tbody>
                          <b-tr>
                            ...
                            <b-td>{{setDate(item.tanggalMulaiPelatihan)}} s/d {{setDate(item.tanggalSelesaiPelatihan)}}</b-td>
                          </b-tr>
                  ...
                  <router-link :to="'/detail_galeri/' + item.id"><b-button variant="primary">Selengkapnya</b-button></router-link>
              </div>
            </div>
          </b-col>
        </b-row>
...
export default {
  ...
  methods:{
    ...
    async getPel(){
      let pelatihan = await axios.get(ipbackend + 'pelatihan/ALLPelatihanSudahSelesai')
